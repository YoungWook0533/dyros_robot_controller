...
namespace RobotController
{
    namespace Mobile
    {
        MobileBase::MobileBase(const double& dt, std::shared_ptr<RobotData::Mobile::MobileBase> robot_data)
        ...
        VectorXd MobileBase::VelocityCommand(const VectorXd& desired_base_vel)
        {
        ...
        VectorXd MobileBase::computeWheelVel(const VectorXd& base_vel)
        {
            assert(base_vel.size() == 3); // Ensure base_vel has three elements for vx, vy, omega       
            return computeIKJacobian() * base_vel;
        }

        MatrixXd MobileBase::computeIKJacobian()
        {
            switch (param_.type) 
            {
                case RobotData::Mobile::DriveType::Differential:
                    return DifferentialIKJacobian();
                case RobotData::Mobile::DriveType::Mecanum:
                    return MecanumIKJacobian();
                case RobotData::Mobile::DriveType::Caster:
        ...
        MatrixXd MobileBase::DifferentialIKJacobian()
        {
        ...
        MatrixXd MobileBase::MecanumIKJacobian()
        {
        ...
        MatrixXd MobileBase::CasterIKJacobian()
        {
