...
HuskyFR3RobotData::HuskyFR3RobotData(const std::string& urdf_path, 
                                     const bool verbose)
...
MatrixXd HuskyFR3RobotData::computeMobileFKJacobian(const VectorXd& q_mobile)
{
...
MatrixXd HuskyFR3RobotData::computeSelectionMatrix(const VectorXd& q)
{
    MatrixXd S;
...
MatrixXd HuskyFR3RobotData::computeSelectionMatrixTimeVariation(const VectorXd& q, const VectorXd& qdot)
{
    MatrixXd S;
    S.setZero(JOINT_DOF,ACTUATOR_DOF);
    S.block(joint_idx.mani_start,actuator_idx.mani_start,MANI_DOF,MANI_DOF).setIdentity();
